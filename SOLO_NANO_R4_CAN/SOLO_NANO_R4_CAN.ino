...
void loop()
{
  ...
  if (communicationFaultLatched &&
    ...
    // During a real CANopen outage SOLO continues using its last received
    // speed. Keep retrying a zero-speed command; as soon as communication
    // returns, command zero and then put the drive into DISABLE (idle). Do not
    // restart automatically while the PWM run request is still present.
