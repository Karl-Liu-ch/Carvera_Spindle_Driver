...
// approximately 625 spindle rpm. Once that threshold is crossed, the motor
...
void updateStartFailureMonitor(uint32_t nowMs)
{
  ...
  if (startFailureFaultLatched || !startFailureMonitorArmed) {
    return;
  }

  const bool encoderTelemetryValid =
    telemetryIsFresh(lastEncoderTimeMs, nowMs);
  if (!odriveDriveEnabled || !odriveCommunicationValid ||
      !encoderTelemetryValid || odriveHasError || odriveFaultLatched ||
      communicationFaultLatched) {
    // Do not charge ODrive startup or invalid telemetry time against the
    // motor, but keep the start attempt armed until conditions are valid.
    startFailureStartTimeMs = 0;
    return;
  }
...
void loop()
{
  ...
  const bool driveFault =
    odriveHasError || odriveFaultLatched || startFailureFaultLatched ||
