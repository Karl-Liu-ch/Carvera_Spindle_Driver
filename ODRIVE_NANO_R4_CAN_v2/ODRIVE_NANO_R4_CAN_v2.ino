...
static constexpr uint32_t BLUE_FEEDBACK_MIN_UPDATE_MS = 100;
static constexpr unsigned int BLUE_FEEDBACK_MIN_CHANGE_HZ = 3;
...
unsigned int blueOutputFrequencyHz = 0;
...
uint32_t lastBlueFeedbackUpdateMs = 0;
...
void updateBlueSpeedFeedback(float motorRpm, bool valid)
{
  ...
  const unsigned int requestedFrequencyHz =
    static_cast<unsigned int>(lroundf(speedFeedbackFrequencyHz));
  const uint32_t nowMs = millis();
  const unsigned int frequencyDifferenceHz =
    requestedFrequencyHz > blueOutputFrequencyHz
      ? requestedFrequencyHz - blueOutputFrequencyHz
      : blueOutputFrequencyHz - requestedFrequencyHz;

  const bool firstStart = !blueFeedbackActive;
  const bool updateDue =
    frequencyDifferenceHz >= BLUE_FEEDBACK_MIN_CHANGE_HZ &&
    nowMs - lastBlueFeedbackUpdateMs >= BLUE_FEEDBACK_MIN_UPDATE_MS;

  // Repeated tone() calls can restart the timer waveform. Keep the existing
  // pulse train running unless a meaningful frequency correction is due.
  if (firstStart || updateDue) {
    tone(BLUE_SPEED_FEEDBACK_PIN, requestedFrequencyHz);
    blueFeedbackActive = true;
    blueOutputFrequencyHz = requestedFrequencyHz;
    lastBlueFeedbackUpdateMs = nowMs;
