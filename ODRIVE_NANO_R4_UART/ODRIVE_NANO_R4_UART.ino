/*
  Carvera PWM -> Arduino Nano R4 -> ODrive UART

  Arduino Nano R4 connections:
    D2  <- Carvera spindle PWM command (~1 kHz)
    D0  <- ODrive TX (Serial1 RX)
    D1  -> ODrive RX (Serial1 TX)
    D3  -> Blue: speed feedback, 12 pulses/revolution (spindle RPM)
    D4  -> Red: alarm, LOW when alarm (0V) / HIGH when normal (5V)
    GND -> Common ground between Carvera, Nano R4 and ODrive

  Configuration:
    - Spindle speed is 1.653 times motor speed
    - PWM input controls spindle RPM, converted to motor RPM by dividing by gear ratio
    - Speed feedback (12 PPR) reports spindle RPM, calculated from motor RPM × gear ratio
    - Maximum commanded spindle RPM: 15000
    - Maximum allowed motor RPM: 11500
    - Direction: CCW (DIRECTION_SIGN = -1.0)

  The Nano R4 GPIO operates at 5 V. Do not use this sketch on an ESP32.
*/
...
void setup()
{
  ...
  Serial.println("Arduino Nano R4 Carvera/ODrive interface started");
}
