/*
  Carvera PWM -> Arduino Nano R4 -> SOLO PICO UART

  Wiring:
    D2  <- Carvera spindle PWM command, approximately 1 kHz
    D0  <- SOLO TX (Serial1 RX)
    D1  -> SOLO RX (Serial1 TX)
    D3  -> Carvera Blue speed feedback, 12 pulses/revolution
    D4  -> Carvera Red alarm, LOW on fault / HIGH when normal

  SOLO, Nano R4 and Carvera must share GND.
...
static constexpr uint8_t RED_ALARM_SIGNAL_PIN = 4;
...
static constexpr float SOLO_CURRENT_LIMIT_A = 10.0f;
...
static constexpr uint32_t SOLO_UART_TIMEOUT_MS = 50;
static constexpr uint8_t SOLO_UART_PACKET_RETRIES = 5;
...
void pollSoloSpeed()
{
  ...
  // feedback through short UART dropouts so Carvera does not see a false
  // spindle-speed loss.
...
void loop()
{
  ...
  // Once a communication fault is latched, stop all normal UART traffic.
  // The dedicated safety-recovery block below is then the only UART caller.
  ...
  if (communicationFaultLatched &&
    ...
    // During a real UART outage SOLO continues using its last received speed.
